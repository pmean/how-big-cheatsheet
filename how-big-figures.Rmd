...
```{r las, fig.width=2.62, fig.height=2.62, echo=FALSE}
par(mar=rep(0,4))
...
Expanding or shrinking the margins and titles.

```{r cex, fig.width=2.62, fig.height=2.62, echo=FALSE}
par(mar=rep(0,4), xaxs="i", yaxs="i")
...
## Drawing a true circle

```{r true_circle, fig.width=7, fig.height=5, echo=FALSE}
plot(1:20, 10*c(1:10,10:1), xlab=" ", ylab=" ")
...
r <- 20
x.circle <- x0 + r / x.px.per.usr * cos(pi.seq)
y.circle <- y0 + r / y.px.per.usr * sin(pi.seq)
lines(x.circle, y.circle, col="red")

r <- 1/20
x.circle <- x0 + r / x.px.per.usr * cos(pi.seq)
y.circle <- y0 + r / y.px.per.usr * sin(pi.seq)
lines(x.circle, y.circle, col="blue")
```
