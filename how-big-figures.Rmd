...
Both the **dev.size** function or **par("din")** will tell you the size of the graphics device in inches (including margins), but **par("din")** will not be updated if you resize your graphics window.

The default in **dev.size** is **units="in"** (inches). Type **dev.size(units="px")** or **dev.size(units="cm")** to get the size in pixels or centimeters.

Like many of the other **par** arguments, **din** is read only (r.o.) meaning that you cannot assign a value to it.
...
Use **par("mai")** to display the size of the four margins (bottom, left, top, right) in inches while **par("mar")** displays the size in lines of text. 

If your axis labels are getting clipped, you can assign wider margins to the **mar** or **mai** parameters.
...
User coordinates are the values you use when plotting your data. Run par("usr") to get a vector of length 4 with the user coordinates with the minimum X value, the maximum X value, the minimum Y value, and the maximum Y value.

This normally is not the same as the values specified by the xlim and ylim arguments in plot. By default, R adds and extra 4% to the plotting range so that points right on the edge do not get partially clipped. You can override this using the xaxs="i" and/or the yaxs="i" arguments in par.
...
Use **par("pin")** to get a vector of length 2 with the size of the plotting region in inches for the X and Y dimensions.

Run **par("plt")** to get the space needed to get to the various edges of the plotting regions from the left/bottom of the graphics 
...
```{r string-size, fig.width=5.25, fig.height=5.25, echo=FALSE}
draw_framework()
text(x(350), y(350), "Abcde\nFghij\nKlmno\nPqrst\nUvwxy")
u0 <- strheight("M")
u1 <- strheight("M\nM") - strheight("M")
u <- y(350)+c(-2, -1, 0, 1, 2)*u1+0.5*u0
u <- c(u, u-u0)
segments(x(308), u, x(392), u, lty="dotted")
draw_framework()
text(x(325), y(375), "Abcde\nFghij\nKlmno", cex=2)
u0 <- strheight("M")
u1 <- strheight("M\nM") - strheight("M")
u <- y(375) + c(2, 2, 0, 0, -2, -2)*u1 + c(1, -1, 1, -1, 1, -1)*u0
segments(x(308), u, x(392), u, lty="dotted")
u12 <- (u[1]+u[2])/2
u34 <- (u[3]+u[4])/2
u56 <- (u[5]+u[6])/2
...
text(x(320), y(340), "W", cex=2)
u <- strwidth("W")
v <- strheight("W")
segments(x(320)-u, y(340)+1.5*v, x(320)-u,  y(340)-2.0*v, lty="dotted")
segments(x(320)+u, y(340)+1.5*v, x(320)+u,  y(340)-2.0*v, lty="dotted")
arrows  (x(320)+u, y(320)-1.5*v, x(320)-u,  y(320)-1.5*v, len=0.05, lwd=3, code=3)

text(x(340), y(340), "L", cex=2)
u <- strwidth("L")
segments(x(340)-u, y(340)+1.5*v, x(340)-u,  y(340)-2.0*v, lty="dotted")
segments(x(340)+u, y(340)+1.5*v, x(340)+u,  y(340)-2.0*v, lty="dotted")
arrows  (x(340)+u, y(340)-1.5*v, x(340)-u,  y(340)-1.5*v, len=0.05, lwd=3, code=3)

text(x(360), y(340), "v", cex=2)
u <- strwidth("v")
segments(x(360)-u, y(340)+1.5*v, x(360)-u,  y(340)-2.0*v, lty="dotted")
segments(x(360)+u, y(340)+1.5*v, x(360)+u,  y(340)-2.0*v, lty="dotted")
arrows  (x(360)+u, y(340)-1.5*v, x(360)-u,  y(340)-1.5*v, len=0.05, lwd=3, code=3)

text(x(380), y(340), "i", cex=2)
u <- strwidth("i")
segments(x(380)-u, y(340)+1.5*v, x(380)-u,  y(340)-2.0*v, lty="dotted")
segments(x(380)+u, y(340)+1.5*v, x(380)+u,  y(340)-2.0*v, lty="dotted")
arrows  (x(380)+u, y(340)-1.5*v, x(380)-u,  y(340)-1.5*v, len=0.05, lwd=3, code=3)
...
segments(x(320)+u, y(320)+1.5*v, x(320)+u,  y(320)-2.0*v, lty="dotted")
arrows  (x(320)+u, y(340)-1.5*v, x(320)-u,  y(340)-1.5*v, len=0.05, lwd=3, code=3)


text(x(340), y(320), "L", cex=2)
segments(x(340)-u, y(320)+1.5*v, x(340)-u,  y(320)-2.5*v, lty="dotted")
segments(x(340)+u, y(320)+1.5*v, x(340)+u,  y(320)-2.5*v, lty="dotted")

text(x(360), y(320), "v", cex=2)
segments(x(360)-u, y(320)+1.5*v, x(360)-u,  y(320)-2.5*v, lty="dotted")
segments(x(360)+u, y(320)+1.5*v, x(360)+u,  y(320)-2.5*v, lty="dotted")
...
text(x(390), y(330), "strwidth", adj=1)
text(x(390), y(310), "cin[1] (in), cra[1] (px), cxy[1] (user)", adj=1)
