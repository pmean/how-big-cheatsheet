...
tbox <- function(x0, y0, s, co="white", cex=1, srt=0, adj=0.5) {
  ...
  polygon(x=x0+tx, y=y0+ty, border="red", col=co)
...
mar <- function() {
  ...
  # left margin
  arrows  (x(200), y(392), x(300), y(392), lwd=2, len=0.05, code=3)
  segments(x(250), y(392), x(250), y(382), lwd=2)
  tbox    (x(250), y(382), "mai[2] (in), mar[2] (lines)", adj=1, srt=90)
...
pin <- function() {
  ...
  arrows(x(308),    y(330), x(300), y(330), lwd=2, len=0.05)
  arrows(x(316)+w3, y(330), x(400), y(330), lwd=2, len=0.05)
  
  s4 <- "plt[1] (pct)"
  w4 <- strwidth(s4)
  text    (x(312), y(310), s4, adj=0)
  arrows  (x(200), y(310), x(300),    y(310), lwd=2, len=0.05)
  segments(x(300), y(310), x(308),    y(310), lwd=2, lty="dotted")
  segments(x(500), y(310), x(316)+w4, y(310), lwd=2, lty="dotted")
  
  s5 <- "plt[2] (pct)"
  w5 <- strwidth(s5)
  text    (x(312),    y(320), s5, adj=0)
  segments(x(200),    y(320), x(308), y(320), lwd=2)
  arrows  (x(316)+w5, y(320), x(400), y(320), lwd=2, len=0.05)
  segments(x(400),    y(320), x(500), y(320), lwd=2, lty="dotted")
  
  s6 <- "pin[2] (in)"
  w6 <- strwidth(s6)
  arrows(x(390), y(371)-w6, x(390), y(300), lwd=2, len=0.05)
  arrows(x(390), y(379),    x(390), y(400), lwd=2, len=0.05)
  ...
  segments(x(380), y(200), x(380), y(371)-w7, lwd=2)
  arrows  (x(380), y(379), x(380), y(400),    lwd=2, len=0.05)
  segments(x(380), y(400), x(380), y(500),    lwd=2, lty="dotted")
  
  s8 <- "plt[3] (pct)"
  w8 <- strwidth(s8)
  text    (x(370), y(375), s8, adj=1, srt=90)
  segments(x(370), y(200), x(370), y(371)-w7, lwd=2, lty="dotted")
  arrows  (x(370), y(200), x(370), y(300),    lwd=2, len=0.05)
  segments(x(370), y(379), x(370), y(500),    lwd=2, lty="dotted")
}
draw_framework()
pin()
```
...
No grpahs needed for this step
...
# Percentage used by plotting region
x.pct <- par("plt")[2] - par("plt")[1]
y.pct <- par("plt")[4] - par("plt")[3]

# Number of pixels in plotting region
x.px <- x.pct*dev.size(units="px")[1]
y.px <- y.pct*dev.size(units="px")[2]

# Number of pixels per user coordinate
x.px.per.usr <- x.px / x.range
y.px.per.usr <- y.px / y.range

# There are some short-cuts
px.per.usr <- par("cra") / par("cxy")

# Step 8a

```{r step8_strwidth, fig.width=3.33, fig.height=3.33, echo=FALSE}
...
```{r step9_panel, fig.width=3.33, fig.height=3.33, echo=FALSE}
...
draw_panel <- function() {
  ...
  # par("fin")
  arrows(x(350), y(305), x(400), y(305), lwd=2, length=0.05, code=3)
  arrows(x(355), y(300), x(355), y(350), lwd=2, length=0.05, code=3)
  ...
  tbox(x(375), y(305), "fin[1] (in)", cex=0.67)
  tbox(x(355), y(325), "fin[2] (in)", cex=0.67, srt=90)
...
Talk about panel graphs here. Roughly two paragraphs.
...
```{r step11_las, fig.width=2.62, fig.height=2.62, echo=FALSE}
par(mar=rep(0,4))
plot(c(100, 200), c(100, 200), type="n", axes=FALSE)
u0 <- 116
u <- u0+c(  0, 13, 26)
v0 <- u0+50
v <- u+50
par(las=2)
axis(side=1, pos=u0, at=u, labels=c("las","=","2"))
axis(side=2, pos=u0, at=u, labels=c("2","=","las"))
par(las=3)
axis(side=1, pos=u0, at=v, labels=c("las","=","3"))
axis(side=2, pos=v0, at=u, labels=c("las","=","3"))
par(las=0)
axis(side=1, pos=v0, at=u, labels=c("las","=","0"))
axis(side=2, pos=u0, at=v, labels=c("las","=","0"))
par(las=1)
axis(side=1, pos=v0, at=v, labels=c("las","=","1"))
axis(side=2, pos=v0, at=v, labels=c("1","=","las"))
```

# Step 11b

```{r step11_cex, fig.width=2.62, fig.height=2.62, echo=FALSE}
par(mar=rep(0,4), xaxs="i", yaxs="i")
plot(c(90, 210), c(90, 210), type="n", axes=FALSE)
u0 <- 127
u <- u0+c(0, 34, 68)
par(las=0)
lb <- c(" cex.axis", "cex.axis", "cex.axis")
axis(side=1, pos=u0, at=u, labels=lb)
axis(side=2, pos=u0, at=u, labels=lb)
text(106, u[2], "cex.lab", srt=90)
text(u[2], 205, "cex.main")
text(u[2], 103, "cex.lab")
text(u[2],  93, "cex.sub")
```
